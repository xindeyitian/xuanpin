...
@property (nonatomic, strong) UIImageView *bothImageView;
...
@property (nonatomic, strong) NSLayoutConstraint *bothCenterYCons;
@end
...
- (void)initializeViews {
    ...
    [NSLayoutConstraint constraintWithItem:self.bothImageView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:10].active = YES;
    [NSLayoutConstraint constraintWithItem:self.bothImageView attribute:NSLayoutAttributeLeading relatedBy:NSLayoutRelationEqual toItem:self.titleLabel attribute:NSLayoutAttributeTrailing multiplier:1 constant:1].active = YES;
    self.bothCenterYCons = [NSLayoutConstraint constraintWithItem:self.bothImageView attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeCenterY multiplier:1 constant:0];
    self.bothCenterYCons.active = YES;
...
- (void)reloadData:(JXCategoryBaseCellModel *)cellModel {
    ...
    if (myCellModel.uiType == JXCategoryTitleSortUIType_ArrowBoth) {
        if (myCellModel.arrowDirection == JXCategoryTitleSortArrowDirection_Both) {
            self.bothImageView.hidden = NO;
            self.bothCenterYCons.constant = 0;
