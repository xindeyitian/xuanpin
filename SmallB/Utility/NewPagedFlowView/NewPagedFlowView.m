...
- (void)layoutSubviews
{
    ...
    NSLog(@"_scrollView = %@, self = %@", NSStringFromCGRect(_scrollView.frame), NSStringFromCGRect(self.bounds));
    _scrollView.frame = CGRectMake((self.width - _pageSize.width) * 0.5, _scrollView.y, _scrollView.width, _scrollView.height);
}
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        switch (self.orientation) {
            case NewPagedFlowViewOrientationHorizontal:
                ...
                NSLog( @"pageIndex = %zd, width = %f", pageIndex, cell.x);
                break;
            case NewPagedFlowViewOrientationVertical:
