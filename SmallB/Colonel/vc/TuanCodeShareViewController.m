...
//    bgWhiteV.clipsToBounds = YES;
    ...
    [bgWhiteV addSubview:bgImageV];
    [bgImageV mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.mas_equalTo(bgWhiteV).insets(UIEdgeInsetsMake(8*KScreenW_Ratio, 8*KScreenW_Ratio, 8*KScreenW_Ratio, 8*KScreenW_Ratio));
    }];
...
- (void)shareBtnClick:(UIControl *)contro{
    ...
    UIImage *image = [self shotShareImageFromView:self.BGWhiteV withHeight:364*KScreenW_Ratio];
    if (contro.tag == 120) {
