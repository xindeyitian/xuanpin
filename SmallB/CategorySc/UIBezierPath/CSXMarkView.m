...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.isVertical) {
        ...
        if (self.isFourCycle) {
            [bezierPath addLineToPoint:CGPointMake(0, self.frame.size.height-self.botDistance-self.speRadius)];
            [bezierPath addArcWithCenter:CGPointMake(0, self.frame.size.height - self.botDistance) radius:self.speRadius startAngle:-M_PI_2 endAngle:M_PI_2 clockwise:YES];
        }
        [bezierPath addLineToPoint:CGPointMake(0, self.frame.size.height-self.borderRadius)];
        [bezierPath addArcWithCenter:CGPointMake(self.borderRadius, self.frame.size.height-self.borderRadius) radius:self.borderRadius startAngle:M_PI endAngle:M_PI_2 clockwise:NO];
        [bezierPath addLineToPoint:CGPointMake(self.frame.size.width-self.borderRadius, self.frame.size.height)];
        [bezierPath addArcWithCenter:CGPointMake(self.frame.size.width-self.borderRadius, self.frame.size.height-self.borderRadius) radius:self.borderRadius startAngle:M_PI_2 endAngle:0 clockwise:NO];
        if (self.isFourCycle) {
            [bezierPath addLineToPoint:CGPointMake(self.frame.size.width, self.frame.size.height-self.botDistance+self.speRadius)];
            [bezierPath addArcWithCenter:CGPointMake(self.frame.size.width, self.frame.size.height - self.botDistance) radius:self.speRadius startAngle:M_PI_2 endAngle:-M_PI_2 clockwise:YES];
        }
...
//    [bezierPath addClip];
    ...
    // 阴影偏移，默认(0, -3)
    pathLayer.shadowOffset = CGSizeMake(0,2);
...
- (void)drawLineOfDashByCAShapeLayer {
    ...
    if (self.isFourCycle) {
        ...
        [shapeLayer setBounds:CGRectMake(0, 0, self.frame.size.width, 1+imaginaryLineWidth)];
        [shapeLayer setPosition:CGPointMake(self.frame.size.width/2,self.frame.size.height - self.botDistance + imaginaryLineWidth)];
        [shapeLayer setFillColor:[[UIColor clearColor] CGColor]];
        // 设置虚线颜色为blackColor
        [shapeLayer setStrokeColor:self.imaginaryLineColor];
        // 3.0f设置虚线的宽度
