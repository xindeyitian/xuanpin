...
/**
 左右两边的两个缺角中心（虚线）距离（水平距离左边，垂直距离顶部）的距离,默认0//圆弧的中心点
 */
@property (nonatomic, assign)float speDistance;
/**
 在有下面两个边角的情况下两个缺角中心（虚线）距离（水平距离左边，垂直距离顶部）的距离,默认0//圆弧的中心点
 */
@property (nonatomic, assign)float botDistance;

/**
 是否有底角两个半圆 默认NO
 */
@property (nonatomic, assign)BOOL isFourCycle;
